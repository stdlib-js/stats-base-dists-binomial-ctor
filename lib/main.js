...
function Binomial() {
	...
	if ( arguments.length ) {
		...
		if ( !isPositiveInteger( n ) ) {
			throw new TypeError( format( '0vt8l,HZ', n ) );
		}
		if ( !isProbability( p ) ) {
			throw new TypeError( format( '0vt8m,Ha', p ) );
		}
	} else {
		n = 1;
		p = 0.5;
	...
	defineProperty( this, 'n', {
		...
		'set': function set( value ) {
			if ( !isPositiveInteger( value ) ) {
				throw new TypeError( format( '0vt8n,HD', value ) );
			}
			n = value;
		}
	});
	defineProperty( this, 'p', {
		...
		'set': function set( value ) {
			if ( !isProbability( value ) ) {
				throw new TypeError( format( '0vt8o,Hb', value ) );
			}
			p = value;
		}
	});
